...
NUM_USED_DECOYS = 1 # number of decoys used for database comparison
...
def _arguments():
    ...
    parser.add_argument("-s", "--skip-rosetta", action='store_true', help="Skip rosetta execution (assume models are already generated)")
...
def _run_para_tmalign(decoy_pdb, templates_pdbs):
    ...
    args = ["paratmalign.sh %d '%s' '%s' %s"%(PROCESSES, decoy_pdb, TMALIGN_SEPARATOR , "'"+"' '".join(templates_pdbs) + "'")]
...
def _run_blast(target_fasta_file, template_fasta_file):
    args = ['blastp', '-query', target_fasta_file, '-subject', template_fasta_file, '-outfmt'] + BLAST_OUTP
    out = _run(args)
...
def main(argv=sys.argv):
    ...
    if not args.skip_rosetta:        
        ...
        abinitio = ["abinitio", "@flags", "-database", rosetta_database, "--nstruct "+str(NUM_DECOYS), "-out:level", "200"]
    ...
    res_pairs = []
    for decoy_id, template_file, strs, seqs, fs in constr_template:
